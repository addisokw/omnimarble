...
def main():
    ...
    s.text("6 stations at z = -60/-40/-20/+5/+60/+120 mm from coil fiducial",
           (30, 20), 2.5)
    s.text("ASSEMBLY VARIANTS (symbol field 'Variant'; per-variant BOMs\n"
           "generated by validate.py -> bom_rail_emit.csv / bom_rail_recv.csv):\n"
           "  EMIT rail: populate Variant=EMIT only (Rx + Dx IR LEDs)\n"
           "  RECV rail: populate Variant=RECV only (Qx phototransistors)\n"
           "Silkscreen carries EMIT/RECV checkboxes to mark the built variant.",
           (30, 28), 2.2)
